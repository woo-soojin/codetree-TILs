...
void teaming(int x, int y, int teamNum) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (grid[nx][ny] == 2 || ((std::make_pair(x, y) != std::make_pair(_x, _y)) && grid[nx][ny] == 3)) {
...
void throwBall(int round) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (grid[sx][sy] > 4) {
            ...
            for (int j = 0; j < teams[teamIdx].size(); j++) {
                if (teams[teamIdx][j].first == sx && teams[teamIdx][j].second == sy) {
                    idx = j + 1;
                    score += idx * idx;
                }
